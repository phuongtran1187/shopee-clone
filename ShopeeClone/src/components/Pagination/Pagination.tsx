...
export default function Pagination({ queryConfig, pageSize }: Props) {
  ...
  const renderPagination = () => {
    ...
    const renderDotAfter = (index: number) => {
      if (!dotAfter) {
        dotAfter = true
        return (
          <span key={index} className='mx-2 rounded bg-white py-2 px-3 shadow-sm'>
            ...
          </span>
        )
      }
      ...
      .map((_, index) => {
        ...
        return (
          <Link
            ...
            key={index}
            className={classNames('mx-2 rounded border bg-white py-2 px-3 shadow-sm', {
  ...
  return (
    <div className='mt-6 flex flex-wrap justify-center'>
      ...
      {renderPagination()}
      {page === pageSize ? (
        <span className='mx-2 cursor-not-allowed rounded border bg-white/60 py-2 px-3 shadow-sm'>Next</span>
      ) : (
        <Link
          to={{
            pathname: path.home,
